export default class TextInput extends window.PIXI.Container {
        ...
        this._input_style = Object.assign(
            {
                ...
                transformOrigin: '0 0',
                lineHeight: '1',
                height: (Number(styles.input.fontSize.replace(/[^+\d]/g, ''))*4)/3//fontSize in pt to size in px
            },
            styles.input
        )

        if (styles.box)
            this._box_generator = typeof styles.box === 'function' ? styles.box : this.DefaultBoxGenerator(styles.box)
        else
    ...
    _updateSurrogateHitbox(bounds: any) {        
    ...
    _updateSurrogateMask(bounds: any, padding: Array<number>) {
    ...
    _updateFontMetrics() {
        ...
        this._font_metrics = window.PIXI.TextMetrics.measureFont(font)
    }
    ...
    _pixiMatrixToCSS(m: any) {
        return 'matrix(' + [m.a, m.b, m.c, m.d, m.tx, m.ty].join(',') + ')'
    }

    _comparePixiMatrices(m1: any, m2: any) {
    ...
    _compareClientRects(r1: any, r2: any) {
    ...
    DefaultBoxGenerator(styles: any) {
        ...
        return function (w: number, h: number, state: string) {
            let style = styles[state.toLowerCase()]
            let box = new window.PIXI.Graphics()
